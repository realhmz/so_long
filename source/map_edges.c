...
/*   Updated: 2024/04/18 17:40:38 by het-taja         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	put_edge(t_vars *v, void **e, int x, int y)
{
	...
	if (v->map[x][y] == 'E')
	{
		mlx_put_image_to_window(v->mlx, v->win, e[i], y * 50 + v->cnsty, x * 50
			+ v->cnstx);
		if (v->c != 0)
			mlx_put_image_to_window(v->mlx, v->win, v->asset->door, y * 50
				+ v->cnsty, x * 50 + v->cnstx);
		else
			mlx_put_image_to_window(v->mlx, v->win, v->asset->open_door, y * 50
				+ v->cnsty, x * 50 + v->cnstx);
	}
	else if (v->map[x][y] == 'C')
	{
		mlx_put_image_to_window(v->mlx, v->win, e[i], y * 50 + v->cnsty, x * 50
			+ v->cnstx);
		mlx_put_image_to_window(v->mlx, v->win, v->asset->coin, y * 50
			+ v->cnsty, x * 50 + v->cnstx);
	}
	else
		mlx_put_image_to_window(v->mlx, v->win, e[i], y * 50 + v->cnsty, x * 50
