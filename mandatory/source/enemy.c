...
void	put_enemy(t_game *game, t_assets *a, int i)
{
	...
	if (i == -1 && j == 1)
	{
		if (game->enemy_moved == 0)
			mlx_put_image_to_window(game->mlx, game->win, a->enemyr[0], game->enemyy * 50
				+ game->cnsty, game->enemyx * 50 + game->cnstx);
		else
			mlx_put_image_to_window(game->mlx, game->win, a->enemyr[game->enemy_moved
				- 1], game->enemyy * 50 + game->cnsty, game->enemyx * 50 + game->cnstx);
	}
	if (i == -1 && x == 1)
	{
		if (game->enemy_moved_left == 0)
			mlx_put_image_to_window(game->mlx, game->win, a->enemyl[0], game->enemyy * 50
				+ game->cnsty, game->enemyx * 50 + game->cnstx);
		else
			mlx_put_image_to_window(game->mlx, game->win,
				a->enemyl[game->enemy_moved_left - 1], game->enemyy * 50 + game->cnsty,
